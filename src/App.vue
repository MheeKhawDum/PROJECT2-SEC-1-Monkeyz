...
//add EACH item from cartItems into orderedItems by calling addItemToOrderedItems() function
function placeOrder() {
  cartItems.value.forEach(addItemToOrderedItems)
  cartItems.value.length = 0 //removes everything in cartItems
...
//add item from cartItems into orderedItems and comparing if it already exists in orderedItems , if it exists it will add quantity instead of adding a new item
function addItemToOrderedItems(item){
  const existingItem = orderedItems.value.find(
    (orderItem) => 
      orderItem.name === item.name &&
      orderItem.drinkType === item.drinkType &&
      orderItem.sweetness === item.sweetness &&
      orderItem.toppings === item.toppings &&
      orderItem.description === item.description
  )

  if (existingItem) {
    existingItem.quantity += item.quantity //adds quantity if the item already exists
    existingItem.totalpriceByOrder += item.totalpriceByOrder //also updates total price
  } else {
    orderedItems.value.push(item)
  }
}

//make it shows 3 of the most ordered custom items
...
//brings the data from orderedItems into customDrink 
function orderSpecialCustomMenu(item) {
  customDrink.value = {... item} // Pre-fill customDrink with item data
...
<template>
  <div class="flex flex-col h-screen">
    ...
    <div name="menu" v-show="menu" class="flex-grow bg-orange-100">
      ...
      <div class="menu-section">
        <h2 class="section-title">Top 3 Ordered Custom Items</h2>
        <div class="menu-slider">
          <div 
            v-for="item in topOrderedItems" 
            :key="item.id" 
            class="menu-item text-black">

            <div v-on:click="orderSpecialCustomMenu(item)">
              <div>
                <img :src="item.image" 
                  :alt="item.name" 
                  class="menu-item-image cursor-pointer drink-options-image" 
